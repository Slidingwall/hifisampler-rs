...
pub fn stft_core(
    ...
    let oxifft_spectrogram = stft(signal, fft_size, hop_size.unwrap_or(consts::HOP_SIZE), WindowFunction::Hann);
...
pub fn istft_core(
    ...
    if n_frames == 0 || freq_bins == 0 || freq_bins != fft_size.unwrap_or(consts::FFT_SIZE) / 2 + 1 {
        return vec![0.0; target_len];
    }
    let mut fft_spec = Vec::with_capacity(n_frames);
    fft_spec.extend((0..n_frames).map(|frame_idx| spec.slice(s![.., frame_idx]).to_vec()));
    let mut result = istft(&fft_spec, hop_size.unwrap_or(consts::HOP_SIZE), WindowFunction::Hann);
