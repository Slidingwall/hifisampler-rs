...
use crate::{
    ...
    utils::{
        cache::{CACHE_MANAGER, Features}, dynamic_range_compression, growl::growl, interp::Akima, interp1d, linspace, midi_to_hz, parser::{pitch_parser, pitch_string_to_midi, tempo_parser, flag_parser}, reflect_pad_2d,
    },
};
const THOP_ORIGIN: f64 = consts::ORIGIN_HOP_SIZE as f64 / consts::SAMPLE_RATE as f64;
const THOP: f64 = consts::HOP_SIZE as f64 / consts::SAMPLE_RATE as f64;
...
impl Resampler {
    ...
    pub fn generate_features(&self) -> Result<Features> {
        ...
        if tension != 0. || breath != voicing {
            ...
            wave = if tension != 0. {
                ...
                let wave_minus_seg: Vec<f64> = wave.iter()
                    .zip(seg_flat)
                    .map(|(&w, &s)| breath_scale * (w - s))
                    .collect();
                ...
                wave.iter()
                    .zip(seg_flat)
                    .map(|(&w, &s)| breath_scale * (w - s) + voicing_scale * s)
                    .collect()
            };
        } else if breath != 100. {
        ...
        let wave_max = wave.iter()
            ...
            .unwrap_or(0.0);
        let (scale, wave) = if wave_max >= 0.5 {
            info!("Scaling audio to max 0.5 (current: {:.3})", wave_max);
            let scale = 0.5 / wave_max;
            (scale, wave.iter().map(|x| x * scale).collect())
        } else {
    ...
    pub fn resample(&self, features: Features) -> Result<()> {
        ...
        let mut t_area_origin: Vec<f64> = (0..mel_origin.ncols())
            .map(|i| i as f64 * THOP_ORIGIN + THOP_ORIGIN / 2.0)
            .collect();
        let mut total_time = t_area_origin.last().copied().unwrap_or(0.0) + THOP_ORIGIN / 2.0;
        ...
        if HIFI_CONFIG.loop_mode || self.flags.contains_key("He") {
            ...
            let start_idx = (((con + THOP_ORIGIN / 2.0) / THOP_ORIGIN)
                .floor() as usize)
                .clamp(0, mel_origin.ncols());
            let end_idx = (((end + THOP_ORIGIN / 2.0) / THOP_ORIGIN)
                .floor() as usize)
                .clamp(start_idx, mel_origin.ncols());
            let mel_loop = mel_origin.slice(s![.., start_idx..end_idx]).to_owned();
            ...
            t_area_origin = (0..mel_origin.ncols())
                .map(|i| i as f64 * THOP_ORIGIN + THOP_ORIGIN / 2.0)
                .collect();
            total_time = t_area_origin.last().copied().unwrap_or(0.0) + THOP_ORIGIN / 2.0;
        ...
        let stretched_n_frames = ((con * vel + (total_time - con) * scaling_ratio) / THOP)
            .floor() as usize + 1;
        let mut stretched_t_mel: Vec<f64> = (0..stretched_n_frames)
            .map(|i| i as f64 * THOP + THOP / 2.0)
            .collect();
        let start_left = ((start * vel + THOP / 2.0) / THOP).floor() as usize;
        let cut_left = start_left.saturating_sub(HIFI_CONFIG.fill);
        let required_frames = ((length_req + con * vel + THOP / 2.0) / THOP).floor() as usize;
        ...
        let t_area_max = t_area_origin.last().copied().unwrap_or(0.0);
        let stretch_t_mel: Vec<f64> = stretched_t_mel
            .iter()
            .map(|&t| stretch(t).clamp(0.0, t_area_max))
            .collect();
        let mel_render = if stretch_t_mel.is_empty() {
            info!("Empty stretched time axis - skipping interpolation");
            Array2::zeros((mel_origin.nrows(), 0))
        } else {
            interp1d(&t_area_origin, &mel_origin, &stretch_t_mel)
        };
        info!("Render mel shape: {:?}", mel_render.dim());
        info!("Processing pitch");
        let pitch_base: Vec<f64> = self.pitchbend.iter()
            .map(|&pb| pb / 100.0 + self.pitch as f64)
            .map(|base| {
                self.flags.get("t")
                    .and_then(|o| o.as_ref())
                    .map(|&t| base + t.clamp(-1200., 1200.) / 100.0) 
                    .unwrap_or(base)
            })
            .collect();
        let pitch_interp = Akima::new(&pitch_base);
        let t_pitch: Vec<f64> = (0..self.pitchbend.len())
            .map(|i| 60.0 * i as f64 / self.tempo + start * vel - cut_left as f64 * THOP)
            .collect();
        let t: Vec<f64> = (0..mel_render.ncols())
            .map(|i| i as f64 * THOP)
            .collect();
        let t_clamped: Vec<f64> = t.iter()
            .map(|&x| x.clamp(*t_pitch.first().unwrap_or(&0.0), *t_pitch.last().unwrap_or(&0.0)))
            .collect();
        let f0_render: Vec<f64> = pitch_interp.sample_with_slice(&t_clamped)
            .iter()
            .map(|&x| midi_to_hz(x))
            .collect();
        ...
        let mut render = {
            ...
            let new_end = new_start + length_req;
            let (start_idx, end_idx) = (
                (new_start * consts::SAMPLE_RATE as f64).floor() as usize,
                (new_end * consts::SAMPLE_RATE as f64).floor() as usize,
            );
            let (start_idx, end_idx) = (
        ...
        info!("Cropped audio length: {}", render.len());
        if let Some(&a_flag) = self.flags.get("A").and_then(|o| o.as_ref()).filter(|&&a| a != 0.0) {
            ...
            if pitch_base.len() > 1 && !t.is_empty() && !render.is_empty() {
                let pitch_deriv: Vec<f64> = pitch_base.iter()
                    .enumerate()
                    .map(|(i, &p)| match i {
                        0 => (pitch_base[1] - p) / (t[1] - t[0]),
                        i if i == pitch_base.len() - 1 => (p - pitch_base[i-1]) / (t[i] - t[i-1]),
                        _ => (pitch_base[i+1] - pitch_base[i-1]) / (t[i+1] - t[i-1]),
                    })
                    .collect();
                let audio_time = linspace(t_clamped[0], t_clamped.last().copied().unwrap_or(0.0), render.len());
                let gain_array = Array2::from_shape_vec((1, pitch_deriv.len()), pitch_deriv)
                    .map_err(|e| anyhow!("Invalid gain array: {}", e))?;
                let mut interpolated_gain = interp1d(&t, &gain_array, &audio_time);
                let (gain_min, gain_max) = (gain_array.column(0)[0], gain_array.column(gain_array.ncols()-1)[0]);
                interpolated_gain.iter_mut().for_each(|g| *g = g.clamp(gain_min, gain_max));
                render.iter_mut()
                    .zip(interpolated_gain.iter())
                    .for_each(|(r, g)| *r *= 5.0f64.powf(1e-4 * a_clamped * g));
        ...
        if let Some(&hg) = self.flags.get("HG").and_then(|o| o.as_ref()) {
            ...
            let hg_clamped = hg.clamp(0.0, 100.0); 
            render = growl(&render, consts::SAMPLE_RATE as i32, 80.0, hg_clamped / 100.0, 10.0);
        }
        if HIFI_CONFIG.wave_norm {
            ...
            render = loudness_norm(&render, consts::SAMPLE_RATE, -1.0, -16.0, p_strength);
        }
        if let Some(max) = render.iter()
            .map(|x| x.abs())
            .filter(|&x| x.is_finite()) 
            .max_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal)) {
            if max > HIFI_CONFIG.peak_limit {
                render.iter_mut().for_each(|x| *x /= max);
            }
        }
