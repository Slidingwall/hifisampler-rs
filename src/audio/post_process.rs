...
pub fn pre_emphasis_base_tension(wave: &[f64], b: f64) -> Vec<f64> {
    ...
    let mut complex_spec_istft = Array2::<Complex<f64>>::zeros((freq_bins, complex_spec.ncols()));
    Zip::from(complex_spec_istft.columns_mut())
        .and(spec_amp_db.axis_iter(Axis(0)))
        .and(spec_phase.axis_iter(Axis(0)))
        .for_each(|mut spec_col, amp_row, phase_row| {
            Zip::from(&mut spec_col)
                .and(amp_row)
                .and(phase_row)
                .for_each(|complex, &amp_db, &phase| {
                    let amp = amp_db.exp();
                    *complex = Complex::new(amp * phase.cos(), amp * phase.sin());
                });
        });
    ...
    filtered_wave.into_iter()
        .take(original_len)
        .map(|x| x * (original_max / filtered_max) * ((b / -15.0).clamp(0.0, 0.33) + 1.0))
        .collect()
}
...
fn linear_fade(length: usize, fade_in: bool, sample_rate: u32) -> Vec<f64> {
    if length == 0 {
        return Vec::new();
    }
    let sr_f64 = sample_rate as f64;
    let fade_len = ((0.2 * sr_f64 + 0.5) as usize)
        .min(length)
        .min(length / 4);
    if fade_in {
...
pub fn loudness_norm(
    ...
    let (mut processed, start_idx, need_restore) = if HIFI_CONFIG.trim_silence {
        let frame_len = (0.02 * sr_f64 + 0.5) as usize;
        let hop_len = (0.01 * sr_f64 + 0.5) as usize;
        let padding_frames = (0.1 * sr_f64 + 0.5) as usize / hop_len;
        let mut start = None;
        let mut end = 0;
    ...
    let min_len = (0.4 * sr_f64 + 0.5) as usize;
...
mod tests {
    ...
    fn test_loudness_norm() {
        ...
        let mut meter = ChannelLoudnessMeter::new(rate);
    ...
    fn test_empty_input() {
        ...
        let normalized = loudness_norm(&empty_signal, 44100, 1.0, -23.0, 100);
    ...
    fn test_edge_cases() {
        ...
        let extreme_quiet = loudness_norm(&signal, 44100, 1.0, -50.0, 100);
        let extreme_loud = loudness_norm(&signal, 44100, 1.0, 0.0, 100);
